        ...
        <div className="max-w-2xl mx-auto flex flex-col gap-10 animate-in fade-in slide-in-from-right-4 duration-300 pb-20">
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                    {currentData.styles.map(style => (
                        <div key={style} className="px-4 py-2 bg-gray-900 text-white rounded-xl text-sm font-bold">
                            {style}
                        </div>
                    ))}
                </div>
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-wrap gap-2 min-h-[48px] p-4 bg-gray-50 rounded-xl border border-gray-100">
                        {currentData.mustGo.map(item => (
                            <div key={item} className="px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg border border-blue-100 text-xs font-bold">
                                {item}
                            </div>
                        ))}
                    </div>
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-wrap gap-2 min-h-[48px] p-4 bg-gray-50 rounded-xl border border-gray-100">
                        {currentData.mustAvoid.map(item => (
                            <div key={item} className="px-3 py-1.5 bg-red-50 text-red-700 rounded-lg border border-red-100 text-xs font-bold">
                                {item}
                            </div>
                        ))}
                    </div>
                ...
                <div className="flex gap-8">
                    {currentData.activeTimes.map(time => (
                        <div key={time} className="flex items-center gap-2">
                            ...
                            <span className="text-gray-700 text-base font-bold">{time}</span>
                        </div>
                    ))}
                </div>
            </div>
            ...
            {currentData.freeText && (
                <div className="flex flex-col gap-4">
                    ...
                    <div className="w-full p-6 bg-gray-50 rounded-2xl border border-gray-100 text-sm text-gray-700 leading-relaxed">
                        {currentData.freeText}
                    </div>
                </div>
            )}
