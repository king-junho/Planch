...
async function loadHealth() {
  healthStatus.textContent = "서버와 데이터베이스 상태를 확인하는 중입니다...";
  ...
  try {
    ...
    healthStatus.textContent = `서버: ${data.status} / 데이터베이스: ${databaseText}`;
  } catch (error) {
    healthStatus.textContent = `상태 확인 실패: ${error.message}`;
  }
}
